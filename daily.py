...
def main(stddate: str, config: ConfigParser) -> None:
    ...
    dtdate = datetime.datetime.strptime(stddate, "%Y-%m-%d").replace(tzinfo=pytz.timezone(config["general"]["timezone"]))
    ...
    for i in range(0, 5):
        ...
        try:
            with open(os.path.join(config["general"]["build_path"], "week-%s.json" % week_start_date.strftime("%Y%m%d")), "r") as week_file:
        ...
        raise FileNotFoundError("Cannot find a week-{}.json file within five prior days")
    ...
    logger.info("Data dumped to " + os.path.join(config["general"]["build_path"], "day-" + date + ".json"))
